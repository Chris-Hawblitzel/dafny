...
namespace Microsoft.Dafny {
  ...
  public class Translator {
    ...
    void AddClassMembers(ClassDecl c)
    {
      ...
      foreach (MemberDecl member in c.Members) {
        ...
        if (member is Field) {
          ...
          isAllocContext = new IsAllocContext(AlwaysUseHeap ? ISALLOC : NOALLOC);
    ...
    void AddIteratorSpecAndBody(IteratorDecl iter) {
      ...
      isAllocContext = new IsAllocContext(MethodsUseHeap ? ISALLOC : NOALLOC);
    ...
    Bpl.Axiom FunctionConsequenceAxiom(Function f, List<Expression> ens) {
      ...
      foreach (Formal p in f.Formals) {
        ...
        wh = GetWhereClause(p.tok, formal, p.Type, etranHeap, NOALLOC);
      ...
      if (!readsHeap) {
        ...
        if (whrAlloc != null) {
          ...
          Bpl.Expr axHeap = BplForall(f.tok, typeParams, formalsHeap, null, trHeap, BplImp(anteIsAlloc, whrAlloc));
    ...
    void AddMethodImpl(Method m, Bpl.Procedure proc, bool wellformednessProc)
    {
      ...
      codeContext = m;
      isAllocContext = new IsAllocContext((m is Lemma ? LemmasUseHeap : MethodsUseHeap) ? ISALLOC : NOALLOC);
    ...
    void TrStmt(Statement stmt, Bpl.StmtListBuilder builder, List<Variable> locals, ExpressionTranslator etran)
    {
      ...
      if (stmt is PredicateStmt) {
        ...
        if (s.AssumeToken == null) {
          ...
          Bpl.Expr wNoAlloc = Bpl.Expr.False;
          Bpl.Expr wIsAlloc = Bpl.Expr.False;
          foreach (var tup in partialGuesses) {
            var bodyNoAlloc = etran.TrExpr(tup.Item2);
            var bodyIsAlloc = etran.TrExpr(tup.Item2);
            if (tup.Item1.Count != 0) {
              var bvsNoAlloc = new List<Variable>();
              var bvsIsAlloc = new List<Variable>();
              var typeAntecedentNoAlloc = etran.TrBoundVariables(tup.Item1, bvsNoAlloc);
              var typeAntecedentIsAlloc = etran.TrBoundVariables(tup.Item1, bvsIsAlloc, false, boundAllocatedContext);
              var triggers = TrTrigger(etran, s.Attributes, s.Tok, substMap);
              bodyNoAlloc = new Bpl.ExistsExpr(s.Tok, bvsNoAlloc, triggers, BplAnd(typeAntecedentNoAlloc, bodyNoAlloc));
              bodyIsAlloc = new Bpl.ExistsExpr(s.Tok, bvsIsAlloc, triggers, BplAnd(typeAntecedentIsAlloc, bodyIsAlloc));
            }
            wNoAlloc = BplOr(bodyNoAlloc, wNoAlloc);
            wIsAlloc = BplOr(bodyIsAlloc, wIsAlloc);
          }
          builder.Add(Assert(s.Tok, wNoAlloc, "cannot establish the existence of LHS values that satisfy the such-that predicate"));
          if (checkAllocated) {
            builder.Add(Assert(s.Tok, wIsAlloc, "cannot prove that LHS values satisfying the such-that predicate are allocated in the current heap." +
    ...
    void TrCallStmt(CallStmt s, Bpl.StmtListBuilder builder, List<Variable> locals, ExpressionTranslator etran, Bpl.Expr actualReceiver) {
      ...
      for (int i = 0; i < s.Lhs.Count; i++) {
        ...
        if (bLhss[i] == null) {  // (in the current implementation, the second parameter "true" to ProcessLhss implies that all bLhss[*] will be null)
          ...
          Bpl.Expr wh = GetWhereClause(lhs.tok, new Bpl.IdentifierExpr(lhs.tok, nm, ty),
            lhs.Type, etran, IsAllocContext.MethodFormalAlloc(s.Method, outFormal));
    ...
    internal class IsAllocContext
    {
      ...
      internal IsAllocContext(IsAllocType defaultAlloc) {
      ...
      internal static IsAllocType MethodFormalAlloc(Method m, Formal formal) {
        ...
        bool isAlloc = mAlloc || (NonGhostsUseHeap && !formal.IsGhost);
      ...
      internal void AddFormal(Formal formal) {
        bool isAlloc = (defaultAlloc == ISALLOC) || (NonGhostsUseHeap && !formal.IsGhost);
        Attributes.ContainsBool(formal.Attributes, "allocated", ref isAlloc);
        locals.Add(formal, isAlloc ? ISALLOC : NOALLOC);
      }

      internal void AddLocalVariable(LocalVariable local) {
        bool isAlloc = (defaultAlloc == ISALLOC) || (NonGhostsUseHeap && !local.IsGhost);
      ...
      internal void AddBoundVariable(BoundVar var) {
        bool isAlloc = (defaultAlloc == ISALLOC) || (NonGhostsUseHeap && !var.IsGhost);
    ...
    public static bool MethodsUseHeap { get { return DafnyOptions.O.Allocated >= 2; } }
    public static bool LemmasUseHeap { get { return DafnyOptions.O.Allocated >= 3; } }
    public static bool AlwaysUseHeap { get { return DafnyOptions.O.Allocated >= 4; } }
